    ...
    _.each(tokens, function (token, i) {
      ...
      case _t.VAR:
        ...
        if (lastState === _t.CURLYOPEN) {
          if (match.length > 1) {
            throw new Error('Unexpected dot on line ' + line + '.');
          }
          out.push(match[0]);
          break;
        }
      ...
      case _t.COLON:
        if (lastState !== _t.CURLYOPEN) {
          throw new Error('Unexpected colon on line ' + line + '.');
        }
        out.push(':');
        break;

      case _t.CURLYCLOSE:
        if (state.pop() !== _t.CURLYOPEN) {
          throw new Error('Unexpected closing curly brace on line ' + line + '.');
        }
        out.push('}');
        break;
