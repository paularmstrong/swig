...
function TokenParser(tokens, filters, autoescape, line, filename) {
  ...
  this.parse = function parse() {
    ...
    utils.each(tokens, function eachTokens(token, i) {
...
TokenParser.prototype = {
   ...
   * @return {undefined}
   */
  on: function onTokenParser(type, fn) {
  ...
  checkMatch: function checkMatch(match) {
    ...
    function checkDot(ctx) {
      ...
      utils.each(m, function eachM(v, i) {
        if (i === 0) {
          return;
        }
 ...
 */
exports.parse = function parse(swig, source, opts, tags, filters) {
  ...
  function parseTag(str, line) {
     ...
     *
     * @example
     * exports.parse = function parse(str, line, parser, types, options, swig) {
     *   parser.on('start', function () {
     *     // ...
     *   });
     *   parser.on(types.STRING, function (token) {
   ...
   */
  utils.each(source.split(splitter), function eachSource(chunk) {
 ...
 */
exports.compile = function compile(template, parents, options, blockName) {
  var out = '',
    tokens = utils.isArray(template) ? template : template.tokens;

  utils.each(tokens, function eachTokens(token) {
    var o;
    if (typeof token === 'string') {
