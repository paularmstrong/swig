...
function remapBlocks(parent, template) {
  return utils.map(parent.tokens || parent, function (token) {
    ...
    if (token.content && token.content.length) {
      token.content = remapBlocks(token.content, template);
    }
    return token;
  });
}
...
exports.compile = function (template, parent, options, blockName) {
  ...
  if (parent && template.blocks) {
    tokens = remapBlocks(parent, template);
  }
