...
function getParentBlock(token) {
    ...
    if ( token.parentBlock ) {
        ...
        _.any( token.parent, function (parent, index) {
                block = getParentBlock( parent );
                
                // return first encountered block in parent tree
                if ( block !== undefined ) {
                    return true;
                }
            }, this);
...
function getIndexInParent(parents, node) {
    return Array.prototype.indexOf.call(parents, node);
}
            ...
            if (!extendsHasVar) {
                ...
                if (token.name === 'extends') {
                    ...
                    if (this.parent) {
                        ...
                        var parentBlockIndex = _.indexOf(this.parent.tokens, 
                                this.parent.blocks[blockname]);

                        token.parentBlock = this.parent.tokens[parentBlockIndex];
                        this.parent.tokens[parentBlockIndex] = token;

                        //_.each(this.parent.tokens, function (curToken, curIndex) {
                        //        if (curToken.name === 'block' && blockname === curToken.args[0]) {
                        //            // Store parent token list in tree
                        //            this.parent.tokens[curIndex] = token;
                        //        }
                        //    }, this);
                    }
...
function postcompile(indent, context, template) {
    var code = '',
        wrappedInMethod;
    ...
    _.each(this.tokens, function (token, index) {
        ...
        if (token.name === 'block') {
            ...
            code += exports.compile.call(template.blocks[token.args[0]], indent + '  ', null, context, template );
        } else if (token.name === 'parent') {
            var parentBlock = getParentBlock(token);
            if (!parentBlock) {
                throw new Error('No parent block found for parent tag at line ' +
                                token.line + '.');
            }

            code += exports.compile.call(parentBlock, indent + '  ', null, context);
            ...
            code += exports.compile.call(token, indent + '  ', null, context);
        }
...
exports.compile = function compile(indent, deprecated, context, template) {
    indent = indent || '';
    ...
    return postcompile.call(this, indent, context, template);
};
