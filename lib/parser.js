...
function precompile(indent, context) {
    ...
    if (this.type === TEMPLATE) {
        ...
        _.each(this.tokens, function (token, index) {
            ...
            if (!extendsHasVar) {
                ...
                if (token.name === 'extends') {
                    ...
                    this.precompiledTokens = this.parent.tokens;
                    ...
                    // store blocks as flat reference list on top-level template object
                    this.blocks[blockname] = token;

                    // child tokens may contain more blocks at this template level - apply to flat this.blocks object
                    findSubBlocks(token, this.blocks);

                    // search precompiledToken list for a matching block, replacing the parent template block
                    // tokens with the derived token.
                    _.each(this.precompiledTokens, function (curToken, curIndex) {
                            if (curToken.name === 'block' && blockname === curToken.args[0]) {
                                // Store precompiled token list in tree
                                this.precompiledTokens[curIndex] = token;
                            }
                        }, this);
        ...
        if (this.precompiledTokens) {
            this.tokens = this.precompiledTokens;
        }
...
function postcompile(indent, parentBlock, context, template) {
    var code = '',
        wrappedInMethod;
    ...
    _.each(this.tokens, function (token, index) {
        ...
        if (token.name === 'block') {
            ...
            code += exports.compile.call(template.blocks[token.args[0]], indent + '  ', parentBlock, context, template );
...
exports.compile = function compile(indent, parentBlock, context, template) {
    indent = indent || '';

    // Template parameter is optional (not used at the top-level).  
    if (this.type === TEMPLATE) {
        template = this;
    }

    // Precompile step - process block inheritence into token hierarchy
    precompile.call(this, indent, context);

    // True compilation step - process all token hierarchy and output code.
    return postcompile.call(this, indent, parentBlock, context, template);
};

// For nesting support:
// 1.) Update parser to make sure it tokenizes nested blocks where a block is read as a single token.
//     - NSS: I think this works as-is.
// 2.) Update the compile code to support nesting.
//     - The compile() function is recursively called when a block is encountered.
//       Change the "block" precompile step to return a more complex "block" object rather than a simple string
//       containing the code (with _output +='s).  
//     - Later, in the token processing loop, change the compile step to render the more complex "block" object
//       into code correctly.  Right now it just +='s the code string for a block.
