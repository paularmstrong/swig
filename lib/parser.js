...
exports.parse = function (swig, source, opts, tags, filters) {
  ...
  function parseTag(str, line) {
     ...
     *   });
     * 
     *   // see https://github.com/paularmstrong/swig/blob/master/lib/lexer.js for 
     *   // available types, or use '*' to catch every token
     *   // `token.match` contains the matched string
     *   parser.on(types.STRING, function (token) {
     *     // persist state to the next token if necessary. `this.state` is an 
     *     // empty array by default but can be overwritten with any value
     *     if (token.match === 'debug') {
     *        this.state = {debug: true};
     *     }
     *     // or send something to the compiler
     *     this.out.push(token.match)
     *   });
     *   parser.on('end', function () {
     *     // content can be sent after all tokens have been handled, making use
     *     // of accumulated state
     *     this.out.push(somethingElse)
     *   });
     * };
     *
     ...
     * @property {array} [args] Array of arguments for the tag. These comprise of everything that is passed to this.out.push() in the parser
