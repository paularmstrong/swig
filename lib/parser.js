...
exports.parse = function (source, opts, tags, filters) {
  ...
  utils.each(source.split(splitter), function (chunk) {
    ...
    if (stripPrev && tokens.length) {
      ...
      } else if (prevToken.content && prevToken.content.tokens) {
        prevChildToken = stripPrevToken(prevToken.content.tokens.pop());
        prevToken.content.tokens.push(prevChildToken);
      }
...
/**
 * Compile a list of parsed tokens into a renderable function.
 * @param  {object} template  Parsed template.
 * @param  {object} parent    Parsed template that is the direct parent of the incoming template.
 * @param  {object} options   Swig options object.
 * @param  {string} [blockName] Name of the current block being compiled.
 * @return {string}           String source of a JavaScript function that can render the given template.
 */
