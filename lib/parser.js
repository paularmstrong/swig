...
function getTokenArgs(args) {
    ...
    function concat(from, ending) {
        ...
        while (!(end).test(arg) && i < args.length) {
            out += ' ' + args[i];
            args[i] = null;
            i += 1;
        }
    ...
    for (; i < l; i += 1) {
        ...
        if (((/^\"/).test(arg) && !(/\"$/).test(arg)) || ((/^\'/).test(arg) && !(/\'$/).test(arg))) {
            out.push(concat(i, arg.substr(0, 1)));
            continue;
        }
    ...
    for (; i < j; i += 1) {
        ...
        if (token.length === 0 || commentRegexp.test(token)) {
            ...
            token = {
                ...
                line: curline,
                name: tagname,
                args: parts.length ? getTokenArgs(_.map(parts, doubleEscape)) : [],
