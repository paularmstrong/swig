...
//	console.log(token, match, lastState, prevToken);
    ...
    case _t.PARENOPEN:
      if (self.filterApplyIdx.length) {
        ...
        if (prevToken && prevToken.type === _t.VAR) {
          ...
          self.out.push(' || _fn).call(' + this.checkMatch(temp));
          self.out.push(',');
          token._fnCtx = true;
        } else {
          self.out.push(' || _fn)(');
    ...
    case _t.PARENCLOSE:
      ...
      if (typeof prevToken._fnCtx !== "undefined") {
        self.out.pop();
      }
      self.out.push(')');
  ...
  parseVar: function (token, match, lastState, prevToken) {
    ...
    self.out.push(this.checkMatch(match));
  },
