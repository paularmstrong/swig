...
exports.compile = function compile(compiler, args, content, parents, options, blockName) {
  ...
  return '_ctx.' + fnName + ' = function (' + args.join('') + ') {\n' +
    ...
    '  _utils.each(_ctx, function each_ctx(v, k) {\n' +
...
exports.parse = function parse(str, line, parser, types) {
  ...
  parser.on(types.FUNCTION, function onFunction(token) {
    if (!name) {
      name = token.match;
      this.out.push(name);
  ...
  parser.on(types.FUNCTIONEMPTY, function onFunctionempty(token) {
    if (!name) {
      name = token.match;
      this.out.push(name);
    }
  ...
  parser.on(types.PARENCLOSE, function onParenclose() {
    if (this.isLast) {
      return;
    }
  ...
  parser.on(types.COMMA, function onComma() {
    return true;
  });

  parser.on('*', function onAnything() {
    return;
  });
