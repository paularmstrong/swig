 ...
 */
exports.compile = function compile(compiler, args) {
  return args.join(' ') + ';\n';
};

exports.parse = function parse(str, line, parser, types) {
  var nameSet = '',
    propertyName;

  parser.on(types.VAR, function onVar(token) {
    if (propertyName) {
  ...
  parser.on(types.BRACKETOPEN, function onBracketopen(token) {
  ...
  parser.on(types.STRING, function onString(token) {
    if (propertyName && !this.out.length) {
      propertyName += token.match;
      return;
    }
  ...
  parser.on(types.BRACKETCLOSE, function onBracketclose(token) {
    if (propertyName && !this.out.length) {
  ...
  parser.on(types.DOTKEY, function onDotkey(token) {
  ...
  parser.on(types.ASSIGNMENT, function onAssignment(token) {
