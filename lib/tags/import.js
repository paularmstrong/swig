...
exports.compile = function (compiler, args, content, parents, options) {
  ...
  utils.each(tokens, function (token) {
    ...
    out += ctx + '.' + macroName + ' = ' + token.compile(compiler, token.args, token.content, parents, utils.extend({}, options, { resolveFrom: parentFile }));
    safe = macroName + '\\.safe ';
    out = out.replace(new RegExp(safe), ctx + '.' + safe.replace(/\\/g, ''));
  });
