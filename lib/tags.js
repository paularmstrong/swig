...
exports.include = function (indent) {
    ...
    indent = indent || '';
    // console.log('ARGS', args, template, !helpers.isLiteral(template), !helpers.isValidName(template))
    ...
    if (args.length) {
        ...
        if (args[0] === 'with') {
            ...
            _.each(args, function (value, key) {
                context += '"' + value + '": __context["' + value + '"] || ' + value + ',';
            });
            context += ' })';
        }
    }
    ...
    return [
        ...
        '        __output += __this.compileFile(__template).render(includeContext, __parents);',
        '    }',
        '})();'
    ].join('\n' + indent);
};
...
exports['for'] = function (indent) {
    ...
    out = '(function () {\n' +
        ...
        '        }\n' +
        '    }\n' +
        '    __context["' + operand1 + '"] = __origOperand1Value;\n' +
        '})();\n';
