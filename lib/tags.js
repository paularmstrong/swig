...
exports['if'] = function (indent) {
    ...
    out.push('   __op1 = ' + helpers.wrapFilters(operand1.name, operand1.filters, null, false)  + ';');
    out.push(' }');
    out.push(' else if (' + check(operand1.name, '__context') + ') {');
    out.push('   __op1 = ' + helpers.wrapFilters(operand1.name, operand1.filters, '__context', false) + ';');
    out.push(' }');
    if (operand2.name === '') {
        ...
        out.push('   __op2 = ' + helpers.wrapFilters(operand2.name, operand2.filters, null, false) + ';');
        out.push(' }');
        out.push(' else if (' + check(operand2.name, '__context') + ') {');
        out.push('   __op2 = ' + helpers.wrapFilters(operand2.name, operand2.filters, '__context', false) + ';');
        out.push(' }');
...
exports['for'] = function (indent) {
    ...
    , '     __forloopIter;'
    , ' if (' + check(operand2) + ') {'
    , '     __forloopIter = ' + escape(operand2) + ';'
    , ' } else if (' + check(operand2, '__context') + ') {'
    , '     __forloopIter = ' + escape(operand2, '__context') + ';'
    , ' } else {'
    , '     return;'
    , ' }'
