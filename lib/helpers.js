...
var key = ['Array', 'RegExpt', 'Object', 'String', 'Number', 'Math', 'Error', 'break', 'continue', 'do', 'for', 'new', 'case', 'default', 'else', 'function', 'in', 'return', 'typeof', 'while', 'delete', 'if', 'switch', 'var', 'with'];
...
function isValidName(string) {
    return string.substr(0, 2) !== '__' && (/^([$A-Za-z_]+[$A-Za-z_0-9]*)(\.?([$A-Za-z_]+[$A-Za-z_0-9]*))*$/).test(string) && _.indexOf(key, string) !== -1;//&& !KEYWORDS.test(string);
}
exports.isValidName = isValidName;

// Variable names starting with __ are reserved.
function isValidShortName(string) {
    return string.substr(0, 2) !== '__' && (/^[$A-Za-z_]+[$A-Za-z_0-9]*$/).test(string) && _.indexOf(key, string) !== -1;//&& !KEYWORDS.test(string);
}
