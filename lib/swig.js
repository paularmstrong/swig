...
function renderFuncBody(code) {
  // Create the body of a function as a string
  return [
...
function createRenderFunc(code) {
  ...
  return new Function('_context', '_parents', '_filters', '_', '_ext', renderFuncBody(code) );
}
...
function createTemplate(data, id) {
  ...
  // Store the parsed code for precompilation
  template.code = code;
...
// Allow registering of a precompiled template, which should be the result
// of swig.precompile(...)
// If you have a render function, use swig.registerTemplate('template-id',
// function)
exports.registerTemplate = function(id, render) {
  var template = {
      // Allows us to include templates from the compiled code
      compileFile: exports.compileFile,
      // These are the blocks inside the template
...
exports.precompile = function (source, options) {
  ...
  return 'function(_context, _parents, _filters, _, _ext) { ' + renderFuncBody(tmpl.code) + ' }';
};
