...
exports.Swig = function (opts) {
  ...
  this.compile = function (source, options) {
    ...
    if (tokens.parent) {
      if (!key) {
        throw new Error('Cannot extend "' + tokens.parent + '" because current template has no filename.');
      }

      // TODO: do we need an alternate solution for in-browser?
      parentFile = path.resolve(path.dirname(key), tokens.parent);
    ...
    function compiled(context) {
      ...
      return tpl(_.extend({}, locals, context || {}));
    }
