...
exports.Swig = function (opts) {
  ...
  /**
   * Add a custom "simple" tag.
   * Inspired by Django's `simple_tag`, `setSimpleTag()` is a streamlined way to define basic custom tags. These tags never use an end tag, nor do they work outside of a block. All arguments passed to the tag in the template are provided to the handler function for processing.
   *
   * Note that only number/string/variable parameters are supported inside the tag, and tag output is *not* escaped by default (although you can do this yourself inside the tag's logic).
   *
   * @example
   * swig.setSimpleTag('printSum', function(arg1, arg2) { return arg1 + arg2; });
   * // => {% printSum 1 2 %}
   * // => 3
   *
   * @param  {string} name      Tag name.
   * @param  {function} handler   Performs the tag's custom logic, accepting the parsed args.
   * @return {undefined}
   */
  this.setSimpleTag = function (name, handler) {
    if (typeof handler !== 'function') {
      throw new Error('Tag "' + name + '" handler method is not a valid function.');
    }
    ...
    tags[name] = {
      parse: function (str, line, parser, types, stack, options) {
        parser.on(types.NUMBER, function (token) {
          this.out.push(parseFloat(token.match));
        });

        parser.on(types.STRING, function (token) {
          // Remove leading/trailing quotes from string literal:
          var match = token.match.match(quotesRegex);
          this.out.push(match[1]);
        });

        parser.on(types.VAR, function (token) {
          this.out.push(options.locals[token.match]);
        });
      ...
      compile: function (compiler, args, content, parents, options, blockname) {
        var result = handler.apply(null, args),
          resultIsNil = (result === null) || (result === undefined),
          output = resultIsNil ? '' : String(result);
...
exports.setSimpleTag = defaultInstance.setSimpleTag;
