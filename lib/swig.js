...
function validateOptions(options) {
  ...
  utils.each(['varControls', 'tagControls', 'cmtControls'], function eachControls(key) {
    ...
    utils.each(options[key], function eachOption(a, i) {
      if (a.length < 2) {
 ...
 * @return {undefined}
 */
exports.setDefaultTZOffset = function setDefaultTZOffset(offset) {
 ...
 */
exports.Swig = function Swig(opts) {
   ...
   * @return {undefined}
   */
  this.setFilter = function setFilter(name, method) {
   ...
   * @return {undefined}
   */
  this.setTag = function setTag(name, parse, compile, ends, blockLevel) {
   ...
   * @return {undefined}
   */
  this.setExtension = function setExtension(name, object) {
   ...
   * @private
   */
  this.parse = function parse(source, options) {
    validateOptions(options);
   ...
   * @private
   */
  this.parseFile = function parseFile(pathname, options) {
    var src;
  ...
  function importNonBlocks(blocks, tokens) {
    var temp = [];
    utils.each(blocks, function eachBlocks(block) { temp.push(block); });
    utils.each(temp.reverse(), function eachReverse(block) {
      if (block.name !== 'block') {
   ...
   */
  this.precompile = function precompile(source, options) {
   ...
   */
  this.render = function render(source, options) {
   ...
   * @return {string}             Rendered output.
   */
  this.renderFile = function renderFile(pathName, locals, cb) {
    if (cb) {
   ...
   */
  this.compile = function compile(source, options) {
   ...
   * @return {function}         Renderable function with keys for parent, blocks, and tokens.
   */
  this.compileFile = function compileFile(pathname, options, cb) {
    var src, cached;
   ...
   * @return {string}             Rendered output.
   */
  this.run = function run(tpl, locals, filepath) {
