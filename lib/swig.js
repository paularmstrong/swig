 ...
 * @property {TemplateLoader} loader The method that Swig will use to load templates. Defaults to <var>swig.FileSystem</var>.
 */
var defaultOptions = {
    autoescape: true,
    varControls: ['{{', '}}'],
    ...
    cache: 'memory',
    /**
     * Configure Swig to use either the <var>swig.FileSystem</var> or <var>swig.Memory</var> template loader. Or, you can write your own!
     ...
     * @example
     * // Custom
     * // A custom loader requires two methods: <var>resolve</var> and <var>load</var>.
     * swig.setDefaults({
     *   loader: {
  ...
  if (options.hasOwnProperty('loader')) {
    if (options.loader) {
      if (!options.loader.load) {
        throw new Error('Invalid loader option ' + JSON.stringify(options.loader) + ' found. Expected { load: function (pathname, cb) { ... } }.');
      }
    }
  }
