...
/**
 * check whether a custom value is in an array
 *
 * @example
 * // searchArray = ['one', 'two', 'three'];
 * {{ searchArray | in_array('two') }}
 * // => true
 *
 * {{ searchArray | in_array('four') }}
 * // => false
 * 
 * @param  {array} input
 * @return {boolean} 
 */
exports.in_array = function (searchArray, entity) {
  var out = false;

  if( searchArray.indexOf(entity) >= 0 )
    out = true;
